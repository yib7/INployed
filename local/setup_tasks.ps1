...
function New-DashboardShortcut {
    ...
    $lnk = Join-Path $desktop "INployed Dashboard.lnk"
    ...
    try {
        ...
        $sc.WindowStyle      = 1
        $sc.Description       = "Open the INployed triage dashboard"
        # Use the app's own icon (local/assets/dashboard.ico, generated by
        # make_icon.py). Fall back to pythonw.exe's Python icon if it is missing.
        $Icon = Join-Path $Here "assets\dashboard.ico"
        if (Test-Path $Icon) { $sc.IconLocation = "$Icon,0" }
        else                 { $sc.IconLocation = "$PythonWPath,0" }
        $sc.Save()
