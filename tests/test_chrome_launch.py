"""Chrome profile resolution for opening job/resume links (local/chrome_launch.py).
...
import chrome_launch  # noqa: E402
...
def test_matches_user_name(tmp_path, monkeypatch):
    ...
    assert chrome_launch._chrome_profile_dir("you@example.com") == "Default"


def test_empty_account_returns_default_not_blank_profile(tmp_path, monkeypatch):
    _write_state(tmp_path, _STATE, monkeypatch)
    assert chrome_launch._chrome_profile_dir("") == "Default"


def test_no_match_falls_back_to_default(tmp_path, monkeypatch):
    _write_state(tmp_path, _STATE, monkeypatch)
    assert chrome_launch._chrome_profile_dir("nobody@example.com") == "Default"
...
def test_silent_fallbacks_are_logged_not_printed(tmp_path, monkeypatch, caplog, capsys):
    ...
    with caplog.at_level(logging.WARNING):
        chrome_launch._chrome_profile_dir("nobody@example.com")
        chrome_launch.open_in_chrome("javascript:alert(1)")
...
def test_local_part_fallback(tmp_path, monkeypatch):
    ...
    assert chrome_launch._chrome_profile_dir("you@example.com") == "Profile 9"
...
def test_open_in_chrome_refuses_non_http_url(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(chrome_launch.subprocess, "Popen", lambda *a, **k: calls.append(("popen", a)))
    monkeypatch.setattr(chrome_launch.webbrowser, "open", lambda *a, **k: calls.append(("web", a)))
    for bad in ("--gpu-launcher=calc.exe", "file:///etc/passwd", "javascript:alert(1)", ""):
        chrome_launch.open_in_chrome(bad)
...
def test_open_in_chrome_passes_http_url_after_options_terminator(monkeypatch):
    seen = {}
    monkeypatch.setattr(chrome_launch, "_chrome_launcher", lambda: ("chrome.exe", "Default"))
    monkeypatch.setattr(chrome_launch.subprocess, "Popen",
                        lambda args, *a, **k: seen.setdefault("args", args))
    chrome_launch.open_in_chrome("https://www.linkedin.com/jobs/view/123")
